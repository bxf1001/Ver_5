...
from PyQt5.QtCore import QRunnable, QObject, pyqtSignal , pyqtSlot, QThreadPool,QThread
import warnings
import uuid
...
class WorkerManager(QObject):
    def __init__(self):
        ...
        self.active_workers = []  # List to store active workers

    def start_worker(self, worker):
        self.thread_pool.start(worker)
        self.active_workers.append(worker)  # Append worker to the list

    def cancel_all_workers(self):
        # Iterate over the list of active workers and abort them
...
class WhatsApp(QRunnable):
    ...
    @pyqtSlot()
    def abort(self):
        # Set the aborted flag to True to stop the process
        self.aborted = True
        # Add any cleanup or shutdown procedures here
        # For example, if you have a subprocess running, you can terminate it:
        if hasattr(self, 'subwhatsapp') and self.subwhatsapp:
            self.subwhatsapp.terminate()
            self.subwhatsapp = None
        # Emit a signal if needed to notify the main thread
        self.signals.completed.emit(self.uuid)
...
class UserContactApp(QMainWindow):
    ...
    def start_workers(self, contacts, timers):
        ...
        while not self.worker_queue.empty():
            ...
            worker.abort()  # Ensure workers are aborted if they are running
        ...
        for contact, timer_input in zip(contacts, timers):
            ...
            worker.signals.completed.connect(self.worker_completed)
            worker.signals.timer_progress.connect(self.update_timer_progress_bar)
    ...
    def start_next_worker(self):
        # Start the next worker in the queue
    ...
    def abort_function(self):
        ...
        while not self.worker_queue.empty():
            worker = self.worker_queue.get()
            worker.abort()  # Call the abort method of the worker
        # Reset progress bars
        self.progress_bar.setValue(0)
